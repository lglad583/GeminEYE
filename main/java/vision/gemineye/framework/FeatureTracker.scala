...
/** Detect and track moving state in a series of images.
  *
  * Described in section "Tracking feature points in video", chapter 10.
  *
  * @param maxCount 	maximum number of state to detect
...
class FeatureTracker(maxCount: Int = 500,
  ...
  def process(inputFrame: Mat, outputFrame: Mat): Unit = {
    ...
    if (shouldAddNewPoints) {
      ...
      // add the detected state to the currently tracked state
  ...
  def detectFeaturePoints(grayFrame: Mat): Array[Point2f] = {
    ...
    goodFeaturesToTrack(grayFrame, // the image
      features, // the output detected state
      maxCount, // the maximum number of state
      qLevel, // quality level
      minDist // min distance between two state
    )
